...
bool BuildPreview(std::string& error) {
    ...
    // Stable preview path: avoid the experimental hero shell and source-color raster preview.
    // This keeps the GUI readable while the exported glTF can still be inspected separately.
...
void DrawPreview(HDC hdc, HWND hwnd) {
    ...
    if (!g.hasPreview) {
        DrawTextW(hdc, L"Stable Structured 3D Preview: choose an image, then press Preview 3D.", -1, &title, DT_LEFT | DT_TOP | DT_SINGLELINE);
        return;
    }
    DrawTextW(hdc, L"Stable Structured 3D Preview - auto category + procedural parts", -1, &title, DT_LEFT | DT_TOP | DT_SINGLELINE);
    ...
    HPEN pen = CreatePen(PS_SOLID, 1, RGB(70, 82, 94));
    HGDIOBJ oldPen = SelectObject(hdc, pen);
    HBRUSH brush = CreateSolidBrush(RGB(156, 166, 176));
...
void DoPreview() {
    try {
        EnableUi(false);
        SetStatus("Generating stable structured preview...");
        std::string error;
        ...
        oss << "Preview ready. type=" << make3d::ToString(g.structured.plan.assetType)
            ...
            << ", triangles=" << (g.mesh.indices.size() / 3)
            << ", parts=" << g.structured.plan.parts.size()
            << ". Path=stable structured builder.";
        SetStatus(oss.str());
        EnableUi(true);
        InvalidateRect(g.hwnd, nullptr, TRUE);
...
void DoSave() {
    ...
    try {
        ...
        vertexColorOptions.materialName = "Make3DStableVertexColorMaterial";
        vertexColorOptions.roughnessFactor = 0.78f;
        vertexColorOptions.doubleSided = true;
        make3d::ExportGLTFWithVertexColors(g.mesh, g.image, vertexColorGltfPath, vertexColorOptions, nullptr);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM wParam2) {
    (void)wParam2;
    switch (msg) {
    case WM_CREATE: {
        g.hwnd = hwnd;
        ...
        g.status = CreateWindowW(L"STATIC", L"Ready. Stable path disables experimental hero/color preview.", WS_CHILD | WS_VISIBLE, 16, 205, 900, 64, hwnd, ControlId(ID_STATUS), nullptr, nullptr);
    ...
    case WM_DESTROY:
        ...
        return 0;
    default:
        return DefWindowProcW(hwnd, msg, wParam, wParam2);
    }
}
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow) {
    ...
    InitCommonControlsEx(&icc);
    const wchar_t* kClassName = L"Make3DStableStructuredGuiWindow";
    WNDCLASSW wc{};
    ...
    RegisterClassW(&wc);
    HWND hwnd = CreateWindowExW(0, kClassName, L"Make3D Advanced Engine - Stable Structured Builder", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 980, 740, nullptr, nullptr, hInstance, nullptr);
    if (!hwnd) return 1;
