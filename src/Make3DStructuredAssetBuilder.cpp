...
static void AddEllipsoid(MeshData& mesh, float cx, float cy, float cz, float rx, float ry, float rz, int segments, int rings) {
    segments = std::max(10, segments);
    rings = std::max(5, rings);
    const int base = VertexCount(mesh);
...
static void BuildCharacterStructured(MeshData& mesh, const AssetPlanResult& plan, const StructuredAssetOptions& options) {
    ...
    const float w = std::clamp(plan.targetWidth, 0.72f, 1.35f);
    const float d = std::max(0.42f, plan.targetDepth);
    const int seg = std::max(18, options.radialSegments);

    AddEllipsoid(mesh, 0.0f, h * 0.78f, 0.0f, w * 0.26f, h * 0.18f, d * 0.28f, seg, 10); // torso
    AddEllipsoid(mesh, 0.0f, h * 0.95f, 0.0f, w * 0.19f, h * 0.08f, d * 0.20f, seg, 6); // neck/shoulder blend
    AddEllipsoid(mesh, 0.0f, h * 1.15f, 0.0f, w * 0.31f, h * 0.21f, d * 0.34f, seg, 12); // head
    AddEllipsoid(mesh, 0.0f, h * 1.28f, -d * 0.04f, w * 0.34f, h * 0.09f, d * 0.36f, seg, 6); // hair cap
    AddBox(mesh, 0.0f, h * 0.63f, d * 0.25f, w * 0.56f, h * 0.06f, d * 0.06f); // belt / lower band

    for (float side : {-1.0f, 1.0f}) {
        AddCylinder(mesh, side * w * 0.39f, h * 0.56f, h * 0.89f, 0.0f, w * 0.045f, d * 0.065f, std::max(10, seg / 2));
        AddEllipsoid(mesh, side * w * 0.43f, h * 0.52f, 0.0f, w * 0.075f, h * 0.045f, d * 0.085f, std::max(10, seg / 2), 5);
        AddCylinder(mesh, side * w * 0.13f, h * 0.15f, h * 0.53f, 0.0f, w * 0.055f, d * 0.070f, std::max(10, seg / 2));
        AddFoot(mesh, side * w * 0.13f, h * 0.07f, d * 0.08f, w * 0.18f, h * 0.055f, d * 0.22f, std::max(10, seg / 2));
    }

    // Face details are small raised shapes so the model reads as a stylized character even in clay preview.
...
static void BuildBuildingStructured(MeshData& mesh, const AssetPlanResult& plan, const StructuredAssetOptions& options) {
    const float h = plan.targetHeight;
    const float w = std::clamp(plan.targetWidth, 0.55f, 2.80f);
    const float d = std::max(0.65f, plan.targetDepth);
    ...
    if (options.addDetailParts) {
        const int cols = std::max(2, static_cast<int>(w * 3.0f));
        const int rows = std::max(2, static_cast<int>(h * 2.2f));
        for (int r = 0; r < rows; ++r) for (int c = 0; c < cols; ++c) {
            const float x = -w * 0.35f + (c + 0.5f) * (w * 0.70f / cols);
            const float y = h * 0.25f + (r + 0.5f) * (h * 0.55f / rows);
...
static void BuildFurnitureStructured(MeshData& mesh, const AssetPlanResult& plan, const StructuredAssetOptions& options) {
    const float h = plan.targetHeight;
    const float w = std::clamp(plan.targetWidth, 0.70f, 2.60f);
    const float d = std::max(0.55f, plan.targetDepth);
    const int seg = std::max(10, options.radialSegments / 2);
    AddBox(mesh, 0.0f, h * 0.46f, 0.0f, w, h * 0.12f, d); // seat/tabletop
    AddBox(mesh, 0.0f, h * 0.78f, -d * 0.42f, w * 0.92f, h * 0.48f, d * 0.12f); // back panel
...
static void BuildVehicleStructured(MeshData& mesh, const AssetPlanResult& plan, const StructuredAssetOptions& options) {
    const float h = plan.targetHeight;
    const float w = std::max(1.50f, plan.targetWidth);
    const float d = std::max(0.70f, plan.targetDepth);
    const int seg = std::max(16, options.radialSegments);
...
static void BuildGenericStructured(MeshData& mesh, const AssetPlanResult& plan, const StructuredAssetOptions& options) {
    const float h = plan.targetHeight;
    const float w = std::clamp(plan.targetWidth, 0.45f, 2.40f);
    const float d = std::max(0.42f, plan.targetDepth);
...
StructuredAssetBuildResult BuildStructuredAssetMesh(
    ...
    planOptions.targetHeight = std::max(0.25f, options.targetHeight);
    planOptions.defaultDepth = std::max(0.12f, options.defaultDepth);
