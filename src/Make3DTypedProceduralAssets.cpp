...
static void T(MeshData& m, int a, int b, int c) { if (a!=b && b!=c && c!=a) { m.indices.push_back((std::uint32_t)a); m.indices.push_back((std::uint32_t)b); m.indices.push_back((std::uint32_t)c); } }
...
static void Box(MeshData& m, float cx, float cy, float cz, float sx, float sy, float sz) {
    ...
    V(m,x0,y0,z0,0,0); V(m,x1,y0,z0,1,0); V(m,x1,y1,z0,1,1); V(m,x0,y1,z0,0,1);
    V(m,x0,y0,z1,0,0); V(m,x1,y0,z1,1,0); V(m,x1,y1,z1,1,1); V(m,x0,y1,z1,0,1);
...
static void Cylinder(MeshData& m, float cx, float y0, float y1, float cz, float rx, float rz, int seg) {
    ...
    for (int ring=0; ring<2; ++ring) { float y = ring ? y1 : y0; for (int s=0; s<seg; ++s) { float a = 2.0f*Pi*(float)s/(float)seg; V(m,cx+std::cos(a)*rx,y,cz+std::sin(a)*rz,(float)s/(float)seg,(float)ring); } }
    for (int s=0; s<seg; ++s) { int n=(s+1)%seg; Q(m,b+s,b+n,b+seg+n,b+seg+s); }
    int cb=VC(m); V(m,cx,y0,cz,.5f,.5f); int ct=VC(m); V(m,cx,y1,cz,.5f,.5f);
...
MeshData BuildFurnitureProceduralAsset(const TypedProceduralAssetOptions& o) {
    MeshData m;
    Box(m, 0, 0.78f, 0, 1.25f, 0.16f, 0.72f);
    Box(m, -0.48f, 0.38f, -0.25f, 0.12f, 0.72f, 0.12f);
    Box(m,  0.48f, 0.38f, -0.25f, 0.12f, 0.72f, 0.12f);
    Box(m, -0.48f, 0.38f,  0.25f, 0.12f, 0.72f, 0.12f);
    Box(m,  0.48f, 0.38f,  0.25f, 0.12f, 0.72f, 0.12f);
    if (o.addDetails) Box(m, 0, 1.15f, 0.30f, 1.30f, 0.75f, 0.12f);
    Finish(m, o.targetHeight); return m;
}

MeshData BuildMachineProceduralAsset(const TypedProceduralAssetOptions& o) {
    MeshData m;
    Box(m, 0, .7f, 0, 1.15f, 1.15f, .82f);
    Box(m, 0, 1.34f, 0, 1.0f, .18f, .74f);
    Cylinder(m, -.42f, .12f, .32f, .48f, .15f, .15f, o.radialSegments);
    Cylinder(m,  .42f, .12f, .32f, .48f, .15f, .15f, o.radialSegments);
    if (o.addDetails) { Box(m, 0, .82f, .43f, .65f, .25f, .05f); Box(m, -.38f, .48f, .44f, .18f, .18f, .05f); Box(m, .38f, .48f, .44f, .18f, .18f, .05f); }
    Finish(m, o.targetHeight); return m;
}

MeshData BuildToolProceduralAsset(const TypedProceduralAssetOptions& o) {
    MeshData m;
    Cylinder(m, 0, 0.0f, 1.45f, 0, .08f, .08f, o.radialSegments);
    Box(m, 0, 1.56f, 0, .58f, .18f, .16f);
    if (o.addDetails) { Box(m, -.34f, 1.56f, 0, .16f, .34f, .12f); Box(m, .34f, 1.56f, 0, .16f, .34f, .12f); }
    Finish(m, o.targetHeight); return m;
}
...
MeshData BuildTerrainPieceProceduralAsset(const TypedProceduralAssetOptions& o) {
    ...
    for (int z=0; z<n; ++z) for (int x=0; x<n; ++x) {
        float fx = ((float)x/(float)(n-1)-.5f)*1.8f;
        float fz = ((float)z/(float)(n-1)-.5f)*1.8f;
        float y = 0.10f + 0.16f*std::sin((float)x*.9f) + 0.10f*std::cos((float)z*1.2f);
        V(m, fx, y, fz, (float)x/(float)(n-1), (float)z/(float)(n-1));
    }
    for (int z=0; z<n-1; ++z) for (int x=0; x<n-1; ++x) { int a=base+z*n+x,b=a+1,c=a+n+1,d=a+n; Q(m,a,b,c,d); }
    Box(m, 0, -0.08f, 0, 1.9f, .18f, 1.9f);
    Finish(m, o.targetHeight); return m;
}
